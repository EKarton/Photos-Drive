...
class TestFakeVectorStore(unittest.TestCase):
    ...
    def test_add_media_item_embeddings_adds_embeddings(self):
        req = CreateMediaItemEmbeddingRequest(
            embedding=self._make_embedding(1.0), media_item_id=MOCK_MEDIA_ITEM_ID_1
        )
    ...
    def test_get_relevent_media_item_embeddings_returns_sorted_results(self):
        reqs = [
            CreateMediaItemEmbeddingRequest(
                embedding=np.array([1.0, 0.0]), media_item_id=MOCK_MEDIA_ITEM_ID_1
            ),
            CreateMediaItemEmbeddingRequest(
                embedding=np.array([0.0, 1.0]), media_item_id=MOCK_MEDIA_ITEM_ID_2
            ),
            CreateMediaItemEmbeddingRequest(
                embedding=np.array([0.5, 0.5]), media_item_id=MOCK_MEDIA_ITEM_ID_3
            ),
        ]
        self.store = FakeVectorStore()  # Use 2D embeddings here
        self.store.add_media_item_embeddings(reqs)
